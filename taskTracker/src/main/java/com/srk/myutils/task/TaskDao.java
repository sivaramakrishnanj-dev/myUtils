...
public class TaskDao {
    ...
    public Task add(String title, String dueAt, List<String> tags) throws SQLException {
        String now = LocalDateTime.now().format(FMT);
        try (Connection conn = db.getConnection()) {
            conn.setAutoCommit(false);
            try (PreparedStatement ps = conn.prepareStatement(
                    "INSERT INTO tasks(title,status,due_at,created_at,updated_at) VALUES(?,'TODO',?,?,?)",
                    Statement.RETURN_GENERATED_KEYS)) {
                ps.setString(1, title);
                ps.setString(2, dueAt);
                ps.setString(3, now);
                ps.setString(4, now);
                ps.executeUpdate();
                ...
                conn.commit();
                return new Task(id, title, "TODO", dueAt, now, now, tags);
    ...
    public List<Task> list(String status, String tag, String dueBefore, String dueAfter) throws SQLException {
    ...
    public Task update(int id, String status, String dueAt, List<String> tags) throws SQLException {
        try (Connection conn = db.getConnection()) {
            conn.setAutoCommit(false);
            try {
                String now = LocalDateTime.now().format(FMT);
    ...
    private Task mapTask(Connection conn, ResultSet rs) throws SQLException {
        ...
        return new Task(id, rs.getString("title"), rs.getString("status"),
                rs.getString("due_at"), rs.getString("created_at"), rs.getString("updated_at"), tags);
    }
}
